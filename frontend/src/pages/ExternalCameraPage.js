...
import "../components/Camera/../Visualization/VisualizationCards.css";
...
function ExternalCameraPage() {
  ...
  // Handle RightMenu model selection changes
  useEffect(() => {
    if (isStreaming && rightMenuSelectedModel !== selectedModel) {
      console.log(
        `🔄 Model changed in RightMenu from ${selectedModel} to ${rightMenuSelectedModel}`
      );
      setSelectedModel(rightMenuSelectedModel);

      // Notify backend of model change if WebSocket is open
      if (wsRef.current && wsRef.current.readyState === WebSocket.OPEN) {
        wsRef.current.send(
          JSON.stringify({
            camera_url: cameraUrl,
            model: rightMenuSelectedModel,
  ...
  // Handle Start Streaming button from RightMenu
  useEffect(() => {
    if (rightMenuSettings.startStreaming && !isStreaming) {
      console.log("🎬 Start Streaming triggered from RightMenu");
      ...
      console.log("📹 Starting external camera stream from RightMenu");
      console.log("   Camera URL:", cameraUrl);
      console.log("   Model:", rightMenuSelectedModel);
      ...
      // Reset the flag
      setRightMenuSettings((prev) => ({
        ...prev,
        startStreaming: false,
      }));
    }
  }, [rightMenuSettings.startStreaming, isStreaming]);

  // Handle Stop Streaming button from RightMenu
  useEffect(() => {
    if (rightMenuSettings.stopStreaming && isStreaming) {
      console.log("⏹️ Stop Streaming triggered from RightMenu");
      stopStream();
  ...
  }, [rightMenuSettings.stopStreaming, isStreaming]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <main
          className="external-camera-main"
          style={{
            flex: 1,
            overflowY: "auto",
                ...
                <div style={{ padding: "1rem" }}>
                  ...
                  >
                    ...
                    <p>
                      ...
                      <span
                        style={{ color: autoSwitch ? "#4ade80" : "#dc2626" }}
                      >
                        {autoSwitch ? "ON" : "OFF"}
                      </span>
                    </p>
