// frontend/src/pages/HLSStreamingPage.js
...
const HLSStreamingPage = () => {
  ...
  const startStream = async () => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.detail || `Failed to start stream: ${response.status}`);
      }
      ...
      const manifestUrl = data.manifest_url || `/api/camera/hls/playlist/${data.stream_id}/playlist.m3u8`;
  ...
  const stopStream = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/camera/hls/stop/${streamId}`, {
        method: 'POST'
      });
  ...
  const getStreamStatus = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/camera/hls/status/${streamId}`);
      if (response.ok) {
  ...
  const testConnection = async () => {
    try {
      setError(null);
      const response = await fetch(`${API_BASE_URL}/camera/test-connection?camera_url=${encodeURIComponent(cameraUrl)}`);
