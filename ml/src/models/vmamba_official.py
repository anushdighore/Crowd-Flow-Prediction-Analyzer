...
def load_tmtb_model(checkpoint_path: str, device: Optional[str] = None):
    ...
    try:
        ...
        try:
            from official import model as official_model
            mamba = official_model.mamba
        except ImportError:
            try:
                import model as official_model
                mamba = official_model.mamba
            except ImportError as primary_error:
                raise ImportError(primary_error) from primary_error
        ...
        try:
            ...
            if models_root not in sys.path:
                sys.path.insert(0, models_root)
            from ml.src.models.tmtb.vmamba_tmtb import load_tmtb_model as custom_load
