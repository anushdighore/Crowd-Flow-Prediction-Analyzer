...
from models.tmtb.vmamba import *
from models.tmtb.counting_head import CountingHead
...
class MAMBA4CC(nn.Module):
    def __init__(self, vmamba_path, num_classes):
        ...
        # Only load vmamba backbone weights if path is provided
        if vmamba_path is not None:
...
def mamba(num_classes, vmamba_pretrained_path=None):
    """
    Create MAMBA4CC model.
    
    Args:
        num_classes: Number of output classes
        vmamba_pretrained_path: Path to pretrained VMamba backbone weights.
                                If None, model is created without loading backbone weights.
    """
    model = MAMBA4CC(vmamba_pretrained_path, num_classes)
    return model
