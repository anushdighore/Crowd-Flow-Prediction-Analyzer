"""
CSRNet Crowd Counting API Endpoint

This endpoint uses the CSRNet model from the ML package for crowd counting inference.
"""
...
from pipelines.preprocessing.csrnet_preprocess import CSRNetPreprocessor
from pipelines.postprocessing.csrnet_postprocess import CSRNetPostprocessor
...
def initialize_csrnet():
    """Initialize CSRNet model and processors"""
    global model, device, preprocessor, postprocessor
    
    if model is not None:
        logger.info("CSRNet already initialized")
        return
    
    logger.info("=" * 60)
    logger.info("🚀 Initializing CSRNet Model")
    logger.info("=" * 60)
